...
const SkillSlider: React.FC<{
    ...
    tooltip: string;
    disabled?: boolean;
}> = ({ label, value, onChange, tooltip, disabled }) => (
...
const NewGameScreen: React.FC<NewGameScreenProps> = ({ onStart, onBack, requiresAccountRegistration = false, currentUsername = '' }) => {
  ...
  // New Birth Date States
  const [birthDay, setBirthDay] = useState(1);
  const [birthMonth, setBirthMonth] = useState(0); // 0 = Jan
  ...
  // Initialize Portraits based on gender - NOW PURELY RANDOM 25
  useEffect(() => {
    const genderKey = gender === 'männlich' ? 'male' : 'female';
    // Use generateRandomPortraitSet for 25 fully random images
    const randomSubset = generateRandomPortraitSet(genderKey, 25);
    setAvailablePortraits(randomSubset);
    setCurrentPortraitIndex(0);
    setCustomPortrait(null); // Reset custom portrait on gender change to match standard logic
  }, [gender]);

  const handlePrevPortrait = () => {
      setCustomPortrait(null); // Clear custom on navigation
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        setCustomPortrait(reader.result as string);
      };
  ...
  const handleStartClick = () => {
    if (studioName.trim() === '') {
      setError('Bitte einen Studionamen eingeben.');
      return;
    }

    const resolvedUsername = requiresAccountRegistration ? registerUsername.trim() : currentUsername.trim();
    if (!resolvedUsername || resolvedUsername.length < 3) {
      setError('Kein gueltiger Nutzername gefunden. Bitte erneut einloggen oder registrieren.');
      return;
    }
    ...
    // Determine which portrait ID to send (Custom Base64 or Standard Filename ID)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                {requiresAccountRegistration && (
                 ...
                 <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2 tracking-wider text-center">
                        {t.newGame.gender}
                    </label>
                    ...
                    <div className="relative">
                        <div className="w-32 h-32 bg-gray-700 rounded-full overflow-hidden border-4 border-amber-500 shadow-lg">
                            {customPortrait ? (
                                <img 
                                    src={customPortrait} 
                                alt={t.newGame.customPortraitAlt} 
                        ...
                        {/* Upload Button - now positioned outside the overflow-hidden container */}
                        <div 
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1 tracking-wider">
                    {t.newGame.studioName}
                  </label>
                  <input
                    type="text"
                    value={studioName}
              ...
              <div className="border-l border-gray-700 pl-8 flex flex-col justify-between">
                <div>
                    ...
                    <div className="bg-gray-900/50 p-3 rounded-md text-center mb-4">
                        <p className="text-lg font-bold">{t.newGame.remainingPoints}: <span className={remainingPoints === 0 ? 'text-green-400' : 'text-amber-400'}>{remainingPoints}</span></p>
                    </div>
                    <div className="space-y-4">
                        <SkillSlider label={t.newGame.skillNegotiation} value={negotiationSkill} onChange={handleSliderChange(setNegotiationSkill, charisma, financialSense, filmSense, organizationTalent)} tooltip={t.newGame.skillNegotiationTooltip} />
                        <SkillSlider label={t.newGame.skillCharisma} value={charisma} onChange={handleSliderChange(setCharisma, negotiationSkill, financialSense, filmSense, organizationTalent)} tooltip={t.newGame.skillCharismaTooltip} />
                        <SkillSlider label={t.newGame.skillFinance} value={financialSense} onChange={handleSliderChange(setFinancialSense, negotiationSkill, charisma, filmSense, organizationTalent)} tooltip={t.newGame.skillFinanceTooltip} />
                        <SkillSlider label={t.newGame.skillFilmSense} value={filmSense} onChange={handleSliderChange(setFilmSense, negotiationSkill, charisma, financialSense, organizationTalent)} tooltip={t.newGame.skillFilmSenseTooltip} />
                        <SkillSlider label={t.newGame.skillOrganization} value={organizationTalent} onChange={handleSliderChange(setOrganizationTalent, negotiationSkill, charisma, financialSense, filmSense)} tooltip={t.newGame.skillOrganizationTooltip} />
                    </div>
                </div>

                <div className="mt-6 pt-4 border-t border-gray-600">
                  <p className="text-xs text-gray-400 text-center">
                    Einheitlicher Schwierigkeitsgrad ist aktiv.
                  </p>
                </div>
              </div>
          </div>
