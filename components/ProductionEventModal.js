...
const ProductionEventModal = ({ event, onClose, resolvedEffects }) => {
    ...
    const generateTooltipText = (choiceValue, effect) => {
    ...
    return (_jsx("div", { className: "absolute inset-0 bg-black bg-opacity-70 backdrop-blur-sm flex items-center justify-center z-50 p-4", "aria-modal": "true", role: "dialog", children: _jsxs("div", { className: "bg-gray-800 bg-opacity-95 border border-amber-500 rounded-lg shadow-2xl w-full max-w-2xl p-8", onClick: e => e.stopPropagation(), children: [_jsx("h2", { className: "text-3xl font-bold font-cinzel text-amber-400 mb-6 text-center", children: displayTitle }), _jsx("div", { className: "flex flex-col md:flex-row items-center md:items-start gap-6", children: _jsxs("div", { className: "flex-grow text-center md:text-left w-full", children: [_jsx("p", { className: "text-gray-300 text-lg mb-6 whitespace-pre-wrap", children: displayText }), _jsx("div", { className: "mt-6 flex flex-col gap-4 w-full", children: event.actions.map(action => {
                                    ...
                                    return (_jsxs("div", { className: "relative group w-full", children: [_jsx("button", { onClick: () => onClose(action), className: `${action.className || "bg-amber-500 text-gray-900 font-bold py-3 px-6 rounded-sm uppercase tracking-wider transform hover:bg-amber-400 transition-all duration-300 ease-in-out shadow-lg shadow-amber-500/20"} w-full flex items-center justify-center`, children: buttonLabel }), effectText && (_jsxs("div", { className: "absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-max max-w-xs px-3 py-2 bg-gray-900 border border-gray-600 text-white text-xs font-medium rounded shadow-xl opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50", children: [effectText, _jsx("div", { className: "absolute top-full left-1/2 -translate-x-1/2 w-0 h-0 border-x-4 border-x-transparent border-t-4 border-t-gray-900" })] }))] }, action.value));
                                }) })] }) })] }) }));
};
