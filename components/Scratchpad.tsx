...
const Scratchpad: React.FC = () => {
    ...
    return (
        <div 
            className="fixed flex flex-col shadow-2xl z-[9999]"
            style={{ 
                left: position.x, 
                top: position.y,
        ...
        >
            {/* Header / Drag Handle */}
            <div 
                className="bg-yellow-300 p-2 cursor-move flex justify-between items-center border-b border-yellow-400 rounded-t-lg"
                onMouseDown={handleMouseDown}
            >
                <span className="text-yellow-800 font-bold text-xs uppercase tracking-wider ml-2"></span>
                <button 
                    onClick={handleClose}
                    className="text-yellow-800 hover:text-red-600 font-bold px-2 text-lg leading-none"
                    title="Schließen"
                >
                    ×
                </button>
            ...
            <textarea
                ...
                className="flex-grow p-6 bg-yellow-100 text-gray-800 font-sans text-xl resize-none focus:outline-none rounded-b-lg shadow-inner leading-relaxed"
                placeholder=""
                spellCheck={false}
                style={{ fontFamily: '"Comic Sans MS", "Chalkboard SE", sans-serif' }}
            />
            
            {/* Sticky Note Bottom Shadow Effect */}
            <div className="absolute bottom-0 right-0 w-8 h-8 pointer-events-none bg-gradient-to-tl from-black/20 to-transparent rounded-br-lg"></div>
        </div>
    );
};
