...
const TutorialOverlay = ({ gameState }) => {
    ...
    useEffect(() => {
        if (!playerData || !setPlayerData || !isActive)
            return;
    ...
    const stepData = t.tutorial.steps[currentStep];
    ...
    if (currentStep === 1)
        showNextButton = true;
    if (currentStep === 28)
        isFinished = true; // Last step
    return (_jsxs("div", { className: "fixed bg-gray-900/95 border-2 border-amber-500 rounded-lg shadow-2xl text-white overflow-hidden flex flex-col z-[9999]", style: {
            left: position.x,
            top: position.y,
            width: '32rem',
            maxWidth: '90vw',
            boxShadow: '0 10px 40px rgba(0,0,0,0.7)'
        }, children: [_jsxs("div", { className: "bg-gradient-to-r from-amber-600 to-amber-700 p-3 cursor-move flex justify-between items-center select-none", onMouseDown: onMouseDown, children: [_jsxs("span", { className: "font-bold font-cinzel text-base text-black uppercase tracking-wider flex items-center gap-2", children: [_jsx("span", { className: "text-xl", children: "\u2605" }), " ", showCloseConfirm ? t.tutorial.actions.closeTitle : title] }), !showCloseConfirm && (_jsx("button", { onClick: handleCloseRequest, className: "text-black hover:text-white font-bold px-2 text-xl leading-none", children: "\u00D7" }))] }), _jsx("div", { className: "p-6 text-base text-gray-200 leading-relaxed font-sans", children: showCloseConfirm ? (_jsxs("div", { className: "text-center", children: [_jsx("p", { className: "mb-6 text-amber-200 text-lg", children: t.tutorial.actions.reallyClose }), _jsxs("div", { className: "flex justify-center gap-4", children: [_jsx("button", { onClick: cancelClose, className: "bg-gray-600 hover:bg-gray-500 text-white font-bold py-2 px-6 rounded uppercase transition-colors", children: t.tutorial.actions.cancelClose }), _jsx("button", { onClick: confirmClose, className: "bg-red-600 hover:bg-red-500 text-white font-bold py-2 px-6 rounded uppercase transition-colors", children: t.tutorial.actions.confirmClose })] })] })) : (_jsx("div", { className: "whitespace-pre-wrap", children: content })) }), !showCloseConfirm && (_jsxs("div", { className: "p-4 bg-gray-800 border-t border-gray-700 flex justify-between items-center", children: [_jsxs("span", { className: "text-xs text-gray-500", children: [t.tutorial.actions.step, " ", currentStep, " / 28"] }), (showNextButton || isFinished) && (_jsx("button", { onClick: isFinished ? confirmClose : handleNextClick, className: "bg-amber-500 hover:bg-amber-400 text-black font-bold py-2 px-6 rounded uppercase transition-colors shadow-lg", children: isFinished ? t.tutorial.actions.start : t.tutorial.actions.next }))] }))] }));
};
