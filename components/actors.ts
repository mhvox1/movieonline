...
import { Actor, Genre, TalentTrait } from '../types';
...
export const generateInitialActors = (): Actor[] => {
    const actors: Actor[] = [];
    ...
    ACTOR_DEFINITIONS.forEach((def, index) => {
        ...
        // Kostenberechnung
        let multiplier = 9;
        if (def.skill <= 20) multiplier = 2;
        else if (def.skill <= 50) multiplier = 4;
        else if (def.skill <= 80) multiplier = 6;
