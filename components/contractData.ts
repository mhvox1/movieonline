...
import { ContractOffer, Genre } from '../types';
...
export const generateContractOffers = (count?: number, currentReputation: number = 0): ContractOffer[] => {
    const offers: ContractOffer[] = [];
    const usedStationNames = new Set<string>();
    const usedTitles = new Set<string>(); // Keep local uniqueness for this batch
    ...
    for (let i = 0; i < numOffers; i++) {
        ...
        const genre = genreDeck.pop() as Genre;
