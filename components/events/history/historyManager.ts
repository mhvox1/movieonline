...
const allHistoricalEvents: Record<number, HistoricalEventDef[]> = {
    1990: events1990,
    1991: events1991,
    1992: events1992,
    1993: events1993,
...
export const checkForHistoricalEvent = (currentDate: Date): HistoricalEventDef | null => {
    const year = currentDate.getFullYear();
    const month = currentDate.getMonth(); // 0-11
    const day = currentDate.getDate();

    const yearlyEvents = allHistoricalEvents[year];
    if (!yearlyEvents) return null;
    ...
    for (let i = 0; i < 7; i++) {
        ...
        if (!eventsToList) continue;

        const event = eventsToList.find(e => 
            e.year === checkYear && 
            e.month === checkMonth && 
            e.day === checkDay
        );
