...
import { PlayerData, RandomEvent } from '../../../types';
...
const getBalancedCost = (capital: number, tier: 'low' | 'medium' | 'high'): number => {
    let pct = 0.01;
    if (tier === 'low') pct = 0.005;     // 0.5%
    if (tier === 'medium') pct = 0.02;  // 2.0%
    if (tier === 'high') pct = 0.05;    // 5.0% (Max Cap)
    
    return calculateEventCost(capital, pct);
};

export const DECISION_EVENTS: RandomEvent[] = [
    {
        id: 'dec_01_charity',
        category: 'Studio',
...
const formatMoney = (n: number) => n.toLocaleString('de-DE');

export const resolveDecisionEvent = (eventId: string, choice: string, data: PlayerData): { updatedData: PlayerData, logEntry?: string } => {
    let newData = { ...data };
    let logEntry = '';
    ...
    switch (eventId) {
        ...
        case 'dec_23_haunted':
             ...
             else { logEntry='PR-Stunt draus gemacht. Naja.'; }
             break;
        ...
        case 'dec_27_outsourcing':
             if(choice==='cheap') { 
                 ...
                 } else logEntry=`Hat geklappt. (Kapital: +${formatMoney(costMed)}$)`; 
             }
             else { 
        ...
        case 'dec_30_noise':
             if(choice==='wall') { 
                 ...
                 } else logEntry='Nachbarn beruhigt.'; 
             }
             break;
        ...
        case 'dec_35_espionage':
             if(choice==='buy') { 
                 ...
                 } else logEntry=`Gute Infos bekommen. (Kapital: -${formatMoney(costMed)}$, FP: +150)`; 
             }
             else { logEntry='Abgelehnt.'; }
             break;
        ...
        case 'dec_49_jet':
             ...
             else { logEntry='Abgelehnt. Sparsamkeit.'; }
             break;
