...
const discoverOrGenerateTalent = (data, forcedRole) => {
    ...
    const fittingTalents = pool.filter(t => !t.isDiscovered &&
        t.id !== -1 &&
        !t.isFamily &&
        t.skill >= minSkill &&
        t.skill <= maxSkill);
    let updatedList = [...pool];
    let selectedTalent;
    if (fittingTalents.length > 0) {
        ...
        selectedTalent = generateNewTalent(data.directors, data.actors, undefined, undefined, isDirector ? 'director' : 'actor', true, targetSkill, // Erzwungener Skill-Wert
        data.gameDate);
    ...
    const newData = { ...data };
    if (isDirector) {
        newData.directors = updatedList;
    }
    else {
        newData.actors = updatedList;
    }
