...
import { MovieAwardYear } from '../types';
 ...
 */
export const getMovieAwardDate = (year: number): Date => {
...
export const ALL_FESTIVALS: FestivalDefinition[] = [
    // Static festivals can be defined here if needed for blocking dates
];

export type AwardKey = 'best_film' | 'best_director' | 'best_actor';
...
export const generateInitialMovieHistory = (startYear: number): MovieAwardYear[] => {
    const history: MovieAwardYear[] = [];
