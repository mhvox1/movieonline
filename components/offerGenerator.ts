...
import { ProjectData, Distributor, PlayerData, LifecycleOffer, MovieSize } from '../types';
 ...
 */
const OFFER_CAPS: Record<MovieSize, { qualityCap: number; hypeCap: number }> = {
 ...
 */
const calculateTotalProductionCost = (film: ProjectData): number => {
    if (film.totalCost && film.totalCost > 0) return film.totalCost;
 ...
 */
export const generateLifecycleOffers = (
    film: ProjectData, 
    ...
    for (let i = 0; i < numOffers; i++) {
        ...
        let strategyType: 'cinema_release' | 'direct_to_video' | 'tv_premiere' | 'free_tv_dump';
        ...
        if (distributor.tier >= 4) upfrontRatio = 0.6;
        if (distributor.tier <= 2) upfrontRatio = 0.4;
        ...
        if (qualityRatio >= 0.8) calculatedShare += 0.02;
        if (qualityRatio >= 1.0) calculatedShare += 0.03;

        // Indie-Bonus: Kleine Verleiher geben +5%, um attraktiv zu sein
        if (distributor.tier <= 2) calculatedShare += 0.05;
