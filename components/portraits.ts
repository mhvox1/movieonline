...
import { PORTRAIT_DATA, SkinTone } from './portraitData';
...
export const generateRandomPortraitSet = (gender: 'male' | 'female', count: number = 25): string[] => {
...
export const generateBalancedPortraitSet = (gender: 'male' | 'female', count: number = 10): string[] => {
    const sourceList = gender === 'male' ? PLAYER_MALE_PORTRAITS : PLAYER_FEMALE_PORTRAITS;
    ...
    const pickUnique = (pool: string[], amount: number) => {
    ...
    if (light.length > 0) pickUnique(light, 2);
    if (medium.length > 0) pickUnique(medium, 2);
    if (dark.length > 0) pickUnique(dark, 2);
...
export const getGeneticChildPortrait = (parent1Id?: string, parent2Id?: string): string => {
    ...
    const p1Skin: SkinTone = p1Meta?.skin || 'medium';
    const p2Skin: SkinTone = p2Meta?.skin || 'medium';

    // Logik für Hautfarben-Pool
    let allowedSkins: SkinTone[] = [];
...
export const getNextStagePortrait = (currentPortraitId: string, targetStage: 'toddler'): string => {
    const currentMeta = PORTRAIT_DATA[currentPortraitId];
    
    if (!currentMeta) {
        // Fallback: Zufällig, wenn aktuelles Kind nicht in DB
        if (targetStage === 'toddler') return TODDLER_PORTRAITS[Math.floor(Math.random() * TODDLER_PORTRAITS.length)];
        return '';
    }
    ...
    if (targetStage === 'toddler') {
        ...
        if (pool.length === 0) pool = TODDLER_PORTRAITS;
    }
...
export const getChildToAdultPortrait = (currentPortraitId: string, gender: 'Junge' | 'Mädchen'): string => {
    const currentMeta = PORTRAIT_DATA[currentPortraitId];
...
export const talentPortraits: Record<string, string> = {
