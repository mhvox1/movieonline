...
import { Genre, EmployeeType } from '../../../types';
...
const TrendTab: React.FC = () => {
    ...
    const hasManager = useMemo(() => {
        ...
        if (playerData.employees.some(e => e.type === EmployeeType.Marketingmanager)) return true;
        
        // Check Partner
        if (playerData.partnerIsEmployed && playerData.partnerEmployedAs === EmployeeType.Marketingmanager) return true;
        
        // Check Children
        if (playerData.children.some(c => c.isEmployed && c.employedAs === EmployeeType.Marketingmanager)) return true;
    ...
    if (!isUnlocked) {
        return (
            <div className="w-full h-full flex flex-col items-center justify-center bg-gray-800/80 p-6 rounded-lg shadow-2xl border border-gray-700">
                <div className="bg-gray-900/50 p-8 rounded-xl border border-gray-600 flex flex-col items-center text-center max-w-md">
                    <PieChartIcon className="h-16 w-16 text-gray-600 mb-4" />
                    <h2 className="text-2xl font-bold font-cinzel text-gray-400 mb-4">{t.marketing.trends.lockedTitle}</h2>
                    <p className="text-gray-300 mb-6">
                        {t.marketing.trends.lockedDesc}
                    </p>
                    
                    <div className="w-full space-y-3 text-left bg-black/20 p-4 rounded-lg">
                        <div className="flex items-center justify-between">
                            <span className="text-gray-400">{t.marketing.trends.reqResearch}</span>
                            <span className={hasTech1 ? "text-green-400 font-bold" : "text-red-400 font-bold"}>
                                {hasTech1 ? t.marketing.trends.reqDone : t.marketing.trends.reqMissing}
                            </span>
                        </div>
                        <div className="flex items-center justify-between">
                            <span className="text-gray-400">{t.marketing.trends.reqStaff}</span>
                            <span className={hasManager ? "text-green-400 font-bold" : "text-red-400 font-bold"}>
                                {hasManager ? t.marketing.trends.reqHired : t.marketing.trends.reqMissing}
                            </span>
                        </div>
                    </div>
                </div>
    ...
    const trendData = useMemo(() => {
        if (!playerData.genreTrends) return [];
    ...
    const getTrendStatus = (popularity: number) => {
        if (popularity >= 1.3) return { text: t.marketing.trends.status.hype, color: "text-fuchsia-400" };
        if (popularity >= 1.1) return { text: t.marketing.trends.status.popular, color: "text-green-400" };
        if (popularity <= 0.7) return { text: t.marketing.trends.status.dead, color: "text-red-600" };
        if (popularity <= 0.9) return { text: t.marketing.trends.status.niche, color: "text-orange-400" };
        ...
        <div className="w-full h-full flex flex-col">
            <div className="flex-grow bg-gray-800/80 p-6 rounded-lg shadow-2xl border border-gray-700 overflow-y-auto">
                <div className="flex justify-between items-center mb-6 border-b border-gray-700 pb-4">
                    <div>
                        <h2 className="text-4xl font-bold font-cinzel text-amber-400">{t.marketing.trends.title}</h2>
                        <p className="text-gray-400 text-sm mt-1">{t.marketing.trends.subtitle}</p>
                    </div>
                    <div className="bg-black/30 px-4 py-2 rounded-lg border border-gray-600 text-right">
                        <p className={`text-xs uppercase tracking-wider ${analysisColor}`}>{t.marketing.trends.dataQuality}</p>
                        <p className={`font-bold ${analysisColor}`}>{analysisLabel}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4">
                    {trendData.map((trend) => {
                        ...
                        if (trend.perceivedPopularity >= 1.2) barColor = 'bg-fuchsia-500';
                        else if (trend.perceivedPopularity >= 1.05) barColor = 'bg-green-500';
                        else if (trend.perceivedPopularity >= 0.95) barColor = 'bg-yellow-500';
                        else if (trend.perceivedPopularity >= 0.8) barColor = 'bg-orange-500';
                        else barColor = 'bg-red-600';

                        return (
                            <div key={trend.genre} className="bg-gray-900/50 p-3 rounded-lg border border-gray-700 flex items-center gap-4">
                                <div className="w-32 flex-shrink-0">
                                    <span className="font-bold text-white block truncate">{t.genres[trend.genre]}</span>
                                ...
                                <div className="flex-grow">
                                    <div className="flex justify-end text-xs text-gray-400 mb-1">
                                        <span className="text-gray-400">{t.marketing.trends.demand}</span>
                                    </div>
                                    <div className="w-full bg-gray-800 rounded-full h-3 overflow-hidden relative">
                                        {/* Blur effect for lower tiers to indicate fuzziness visualy */}
                                        {!hasTech3 && (
                                            <div className="absolute inset-0 bg-gray-800/30 backdrop-blur-[1px] z-10"></div>
                                        )}
                                        <div className={`h-full rounded-full transition-all duration-500 ${barColor}`} style={{ width: `${barWidth}%` }}></div>
                                    </div>
                                </div>

                                <div className="w-8 flex-shrink-0 flex justify-center" title={showArrows ? t.marketing.trends.trendDirection : t.marketing.trends.trendTooltip}>
                                    {showArrows ? (
                                        trend.momentum > 0.005 ? <TrendUpIcon /> : trend.momentum < -0.005 ? <TrendDownIcon /> : <TrendFlatIcon />
                                    ) : (
                                        <UnknownIcon />
                                    )}
                                </div>
