...
const CalendarTab: React.FC = () => {
    ...
    const allEvents = useMemo(() => {
        const events: { date: Date, type: string, title: string }[] = [];
        ...
        if (playerData.currentProject) {
            ...
            if (proj.scriptEndDate) events.push({ date: new Date(proj.scriptEndDate), type: 'Drehbuch fertig', title: `"${proj.workingTitle}" ${t.header.events.scriptFinished}` });
            if (proj.castingEndDate) events.push({ date: new Date(proj.castingEndDate), type: 'Casting fertig', title: `"${proj.workingTitle}" ${t.header.events.castingFinished}` });
            if (proj.productionEndDate) events.push({ date: new Date(proj.productionEndDate), type: 'Produktion fertig', title: `"${proj.workingTitle}" ${t.header.events.productionFinished}` });
        }
    ...
    const monthCalendarData = useMemo(() => {
        ...
        const calendarDays: ({ date: Date | null, events: { date: Date; type: string; title: string; }[] })[] = [];
    ...
    return (
        <div className="bg-gray-900/80 p-4 rounded-lg border border-gray-700">
            <div className="flex justify-between items-center mb-4">
                <button onClick={handlePrevMonth} className="p-2 rounded-full hover:bg-gray-700"><ArrowLeftIcon className="h-6 w-6 text-white" /></button>
                <h2 className="text-2xl font-cinzel text-amber-400">{currentMonth.toLocaleString(locale, { month: 'long', year: 'numeric' })}</h2>
                <button onClick={handleNextMonth} className="p-2 rounded-full hover:bg-gray-700"><ArrowRightIcon className="h-6 w-6 text-white" /></button>
            </div>
            <div className="grid grid-cols-7 gap-1 text-center text-xs font-bold text-gray-400 uppercase pb-2 border-b border-gray-600">
                {weekdays.map(day => <div key={day}>{day}</div>)}
            </div>
            <div className="grid grid-cols-7 gap-1 mt-1">
                {monthCalendarData.map((day, index) => (
                    <div key={index} className={`h-28 rounded-md p-1.5 flex flex-col ${day.date ? 'bg-gray-800' : 'bg-gray-800/50'} ${isSameDay(playerData.gameDate, day.date) ? 'border-2 border-amber-400' : 'border border-gray-700'}`}>
                        {day.date && <div className={`font-bold ${isSameDay(playerData.gameDate, day.date) ? 'text-amber-300' : 'text-gray-300'}`}>{day.date.getDate()}</div>}
                        <div className="flex-grow mt-1 space-y-1 overflow-y-auto text-[9px] pr-1 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
