...
const generateInitialSkills = (job) => {
    ...
    if (job.includes('KÃ¼nstler') || job.includes('Musiker') || job.includes('Artist') || job.includes('Musician')) {
        skills.acting += 20;
        skills.writing += 15;
    }
...
export const RelationshipsTab = () => {
    ...
    const handleCommitRelationship = () => {
        ...
        if (hoursKnown < 10) {
            ...
            rejectionMessage = `${playerData.partnerName} fÃ¼hlt sich geschmeichelt, braucht aber noch etwas Zeit.`;
            interestDrop = 10;
        }
        else {
            ...
            if (currentInterest < 50) {
                ...
                rejectionMessage = `${playerData.partnerName} zÃ¶gert noch etwas.`;
            }
        }
        if (isTestMode)
            successChance = 1.0;
    ...
    const handleKinderwunsch = () => {
        ...
        if (playerData.partnerChildrenAgreementCount >= playerData.partnerChildrenAgreementLimit) {
            setRelationshipFeedback(`${playerData.partnerName} mÃ¶chte keine weiteren Kinder mehr.`);
            return;
        }
        setPlayerData(prev => {
            if (!prev)
