...
import { Director, Actor, Genre, PlayerData, TalentTrait, RoleCasting, ActorAge, CastingOption, ProjectData, CastingTalentPreference, ProjectCastingPreferences, TalentQualityPreference } from '../types';
...
const getAgeCategory = (birthDate: Date, gameDate: Date): ActorAge => {
...
const getAdjacentAges = (age: ActorAge): ActorAge[] => {
    switch (age) {
...
const getProjectCastingPreferences = (project: ProjectData): ProjectCastingPreferences => ({
    director: {
...
const getSkillRangeForPreference = (quality: TalentQualityPreference, fallbackMin: number, fallbackMax: number) => {
    switch (quality) {
...
const preferenceHasTargets = (preference: CastingTalentPreference | undefined) => {
    if (!preference) return false;
    return preference.gender !== 'any' || preference.age !== 'any' || preference.quality !== 'any';
};

const matchesGenderPreference = (gender: 'männlich' | 'weiblich', preference: CastingTalentPreference) => {
    return preference.gender === 'any' || gender === preference.gender;
};

const getAgeMatchCandidates = <T extends Director | Actor>(pool: T[], preference: CastingTalentPreference, gameDate: Date) => {
    if (preference.age === 'any') return pool;
...
const generateCastingJustification = (talent: Director | Actor, genre: Genre): string => {
    ...
    // Traits
    if (talent.traits.includes(TalentTrait.Publikumsliebling)) { pros.push(`ist ein ${TalentTrait.Publikumsliebling} und zieht Zuschauer an.`); }
    if (talent.traits.includes(TalentTrait.Arbeitstier)) { pros.push(`ist als ${TalentTrait.Arbeitstier} bekannt, was die Produktion beschleunigt.`); }
    if (talent.traits.includes(TalentTrait.Teamplayer)) { pros.push(`sorgt als ${TalentTrait.Teamplayer} für eine harmonische Atmosphäre am Set.`); }
    if (talent.traits.includes(TalentTrait.Diva)) { cons.push(`neigt zu Allüren als ${TalentTrait.Diva}, was zu Konflikten führen kann.`); }
    if (talent.traits.includes(TalentTrait.Unzuverlässig)) { cons.push(`gilt als ${TalentTrait.Unzuverlässig} und könnte zu Verzögerungen führen.`); }
    if (talent.traits.includes(TalentTrait.Kassengift)) { cons.push(`ist bekannt als ${TalentTrait.Kassengift}, ein Risiko für die Einspielergebnisse.`); }
    
    parts.push(`${talent.name} ist ein${talent.skill > 60 ? ' vielversprechender' : ' solider'} ${role}.`);
    if(pros.length > 0) parts.push(`Pro: ${pronounHeShe} ${pros.join(' Außerdem ')}.`);
    if(cons.length > 0) parts.push(`Contra: ${pronounHeShe} ${cons.join(' Zudem ')}.`);

    return parts.join(' ');
};

const calculateCastingFitScore = (talent: Director | Actor, genre: Genre): number => {
    ...
    if (talent.skill > 80) score += 1.5;
    else if (talent.skill > 60) score += 1;
    else if (talent.skill < 30) score -= 1;

    // Traits (situational impact)
    if (talent.traits.includes(TalentTrait.Publikumsliebling)) score += 0.5;
    if (talent.traits.includes(TalentTrait.Teamplayer)) score += 0.5;
    if (talent.traits.includes(TalentTrait.Arbeitstier)) score += 0.5;
    if (talent.traits.includes(TalentTrait.Diva)) score -= 1;
    if (talent.traits.includes(TalentTrait.Unzuverlässig)) score -= 1.5;
    if (talent.traits.includes(TalentTrait.Kassengift)) score -= 2;
...
export const evaluateCastingPool = (
    directorPoolIds: number[],
    actorPoolIds: number[],
    playerData: PlayerData,
    genre: Genre
): { availableDirectorIds: number[]; availableActorIds: number[]; justifications: { directors: { id: number; text: string; rating: number }[]; actors: { id: number; text: string; rating: number }[] } } => {
    ...
    const justifications: { directors: { id: number; text: string; rating: number; }[]; actors: { id: number; text: string; rating: number; }[] } = {
        directors: [],
        actors: []
    };
...
const createRandomTalent = (
    type: 'director' | 'actor', 
    playerData: PlayerData,
    project: ProjectData,
    talentPreference?: CastingTalentPreference,
    forcedSkill?: number
): Director | Actor => {
    ...
    let gender: 'männlich' | 'weiblich' = isMale ? 'männlich' : 'weiblich';
    ...
    if (availablePortraits.length === 0) availablePortraits = gender === 'männlich' ? ALL_MALE_PORTRAITS : ALL_FEMALE_PORTRAITS;
    ...
    if (talentPreference?.age && talentPreference.age !== 'any') {
        switch(talentPreference.age) {
            case ActorAge.Child: ageInYears = randomBetween(8, 15); break;
            case ActorAge.Young: ageInYears = randomBetween(16, 34); break;
            case ActorAge.MiddleAged: ageInYears = randomBetween(35, 59); break;
            case ActorAge.Old: ageInYears = randomBetween(60, 80); break;
        }
    }
...
export const generateNewTalentsForCastingPool = (
    option: CastingOption, 
    project: ProjectData,
    playerData: PlayerData
): { newActors: Actor[], newDirectors: Director[] } => {
    ...
    const validDirectors = playerData.directors.filter(d => 
        !d.isDiscovered && 
    ...
    for (let i = 0; i < remainingDirectorsNeeded; i++) {
        newDirectors.push(createRandomTalent('director', playerData, project, directorPreference) as Director);
    }
    ...
    const skillValidActors = playerData.actors.filter(a => 
        !a.isDiscovered &&
    ...
    const usedHiddenActorIds = new Set<number>();

    // Helper to find matching hidden actor with fallback logic
    const findBestMatchingActor = (preference: CastingTalentPreference): Actor | null => {
    ...
    for(let i=0; i<numForMainRole; i++) {
        if (preferenceHasTargets(mainActorPreference) || project.mainRole) {
            ...
            if (found) {
                ...
                newActors.push(createRandomTalent('actor', playerData, project, mainActorPreference) as Actor);
            }
        }
    }
    ...
    for(let i=0; i<numForSupportingRole; i++) {
        if (preferenceHasTargets(supportingActorPreference) || project.supportingRole) {
            ...
            if (found) {
                ...
                newActors.push(createRandomTalent('actor', playerData, project, supportingActorPreference) as Actor);
            }
        }
    }
    ...
    for(let i=0; i<numRandom; i++) {
        ...
        if (remainingHidden.length > 0) {
            ...
            newActors.push(createRandomTalent('actor', playerData, project) as Actor);
        }
    }
...
export const generateNewTalent = (
    ...
    const mockProject = { projectPotential: forcedSkill || (scoutTalent ? scoutTalent * 1.5 : 40), scriptQuality: 40 } as ProjectData;
    const mockData = { directors: existingDirectors, actors: existingActors, gameDate: gameDate ? new Date(gameDate) : new Date() } as PlayerData;
    
    // Determine type
    let type: 'director' | 'actor';
    if (role) type = role;
    else type = Math.random() < 0.5 ? 'director' : 'actor';
