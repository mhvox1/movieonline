...
import { PlayerData, BuildingType, Director, Actor, Employee, EmployeeType, ActiveCastingCampaign, ActiveProductionCampaign, ProjectPhase, Message, ActorAge, RoleCasting, Script, ActiveConstruction, ActiveCasting, ActiveTalentScouting } from '../types';
// isSameDay was used for completion checks but can fail if the game date jumps past the end date.
// We'll add a more robust helper and keep isSameDay for other semantics if needed.
import { isSameDay } from './helpers';
import { MARKETING_CAMPAIGNS, PRODUCTION_MARKETING_CAMPAIGNS } from '../components/marketingData';
import { ALL_COURSES, Course } from '../components/privateLifeData';
import { generateNewTalent } from '../components/talentGenerator';
import { useTranslation } from './useTranslation';

// new helper to determine whether the current date has reached or passed a target date
const dateReached = (current: Date, target?: Date | null): boolean => {
...
// Helper to map age to category
const getAgeCategory = (age: number): ActorAge => {
...
interface UseActivityLoopProps {
    playerData: PlayerData;
    setPlayerData: React.Dispatch<React.SetStateAction<PlayerData | null>>;
    systemPause: () => void;
    // setResearchFinishedInfo removed
    setConstructionFinishedInfo: React.Dispatch<React.SetStateAction<{ buildingType: BuildingType; level: number; } | null>>;
    setCampaignResultInfo: React.Dispatch<React.SetStateAction<{ name: string; success: boolean; reputationGained: number } | null>>;
    setProductionCampaignResultInfo: React.Dispatch<React.SetStateAction<{ success: boolean; hypeGained: number; campaignName: string; projectName: string; } | null>>;
    setCourseFinishedInfo: React.Dispatch<React.SetStateAction<{ course: Course } | null>>;
    setTalentScoutingResult: React.Dispatch<React.SetStateAction<{ talent: Director | Actor } | null>>;
    setTrainingFinishedInfo: React.Dispatch<React.SetStateAction<{ talentName: string; message: string; } | null>>;
    setCastingFinishedNotification: React.Dispatch<React.SetStateAction<{ talentName: string; isContinuing: boolean; isGeneral: boolean; } | null>>;
}

export const useActivityLoop = (props: UseActivityLoopProps) => {
    const {
        playerData, setPlayerData, systemPause, setConstructionFinishedInfo,
        setCampaignResultInfo, setCourseFinishedInfo, setTalentScoutingResult,
    ...
    useEffect(() => {
        ...
        // Active Research
        // Prior implementation used isSameDay which missed completions when the gameDate jumped ahead.
        if (newState.activeResearch && newState.activeResearch.progressPoints >= newState.activeResearch.requiredPoints) {
            ...
            // IMPROVEMENT: Researcher gains skill
            if (researcher) {
                newState.employees = newState.employees.map(e => 
                    e.id === researcher.id ? { ...e, talent: Math.min(100, e.talent + 1) } : e
                );
            }
            ...
            dataChanged = true;
            // systemPause(); // Optional: remove pause to be less intrusive, or keep it. Keeping it paused for achievement feeling.
            systemPause();
        }

        // Active Constructions (Iterate all)
        // Ensure activeConstructions exists (migration check)
        if (!newState.activeConstructions) {
            newState.activeConstructions = newState.activeConstruction ? [newState.activeConstruction] : [];
        }

        const finishedConstructions: ActiveConstruction[] = [];
        const remainingConstructions: ActiveConstruction[] = [];
        ...
        if (finishedConstructions.length > 0) {
             finishedConstructions.forEach(finishedConstruction => {
                ...
                // Generate Message instead of Modal
             ...
             // Update legacy pointer for backward compat visualizers if needed, or clear it if empty
             newState.activeConstruction = remainingConstructions.length > 0 ? remainingConstructions[0] : null;
             
             dataChanged = true;
        }
        
        // Active Studio Marketing Campaign
        if (newState.activeMarketingCampaign && dateReached(newDate, newState.activeMarketingCampaign.endDate)) {
            ...
            if (campaign) {
                ...
                // IMPROVEMENT: Marketing Manager gains skill
                // Find a marketing manager to award (first one found)
                const marketer = newState.employees.find(e => e.type === EmployeeType.Marketingmanager);
                if (marketer) {
                    newState.employees = newState.employees.map(e => 
                        e.id === marketer.id ? { ...e, talent: Math.min(100, e.talent + 1) } : e
                    );
                }
        ...
        // Active Production Marketing Campaigns (parallel per project)
        const activeProductionCampaigns = (newState.activeProductionCampaigns && newState.activeProductionCampaigns.length > 0)
        ...
        if (activeProductionCampaigns.length > 0) {
            const remainingCampaigns: ActiveProductionCampaign[] = [];
            ...
            activeProductionCampaigns.forEach((campaignInfo, campaignIndex) => {
                ...
                if (newState.activeProjects) {
                    if (normalizedTargetTitle) {
                        targetProjectIndex = newState.activeProjects.findIndex(p =>
                            p.workingTitle.trim().toLocaleLowerCase() === normalizedTargetTitle
                        );
                    } else {
                        targetProjectIndex = newState.activeProjects.findIndex(p =>
                            (p.phase === ProjectPhase.Production || p.phase === ProjectPhase.PostProduction) &&
                            p.usedProductionCampaigns?.includes(campaignInfo.campaignId)
                        );
                    }
                }
                ...
                if (!shouldEnd) {
                    remainingCampaigns.push(campaignInfo);
                    return;
                }
                ...
                if (marketerId !== -1 && marketerId < 90000) {
                    newState.employees = newState.employees.map(e =>
                        e.id === marketerId ? { ...e, talent: Math.min(100, e.talent + 1) } : e
                    );
                }
            ...
            if (remainingCampaigns.length !== activeProductionCampaigns.length) {
                newState.activeProductionCampaigns = remainingCampaigns;
            ...
            if (course) {
                if (course.skillBonus) {
                    ...
                    if (typeof newState[skill] === 'number') {
                        // @ts-ignore
                        newState[skill] = Math.min(100, (newState[skill] as number) + amount);
                    }
                }
        ...
        // NEW: Active Seminar / Leisure (Player)
        if (newState.activeSeminar && dateReached(newDate, newState.activeSeminar.endDate)) {
            ...
            dataChanged = true;
            // No pause needed for short activities, but feedback is given via log/stats update
        }

        // Casting Missions (Multiple)
        // Ensure activeCastings exists
        if (!newState.activeCastings) {
            newState.activeCastings = newState.activeCasting ? [newState.activeCasting] : [];
        }

        const remainingCastings: ActiveCasting[] = [];
        ...
        newState.activeCastings.forEach(casting => {
            if (dateReached(newDate, casting.endDate)) {
                ...
                // IMPROVEMENT: Caster gains skill
                if (casterId) {
                    newState.employees = newState.employees.map(e => 
                        e.id === casterId ? { ...e, talent: Math.min(100, e.talent + 1) } : e
                    );
                }
                ...
                // Helper to update fame
                const updateFame = (t: Director | Actor) => {
                    if (t.id === talentId) {
                        // Increase fame by 1, max 5
                        const newFame = Math.min(5, t.bekanntheit + 1);
                        currentFame = newFame;
                        return { ...t, bekanntheit: newFame };
                    }
                    return t;
                };

                newState.directors = newState.directors.map(d => updateFame(d) as Director);
                newState.actors = newState.actors.map(a => updateFame(a) as Actor);

                let isContinuing = false;
                let nextTargetId = talentId;
                let nextTargetName = talentName;

                // Determine if and with whom we continue
                if (isGeneralCasting) {
                    // If the current talent is NOT yet at 5, we continue with THE SAME talent
                    if (currentFame < 5) {
                        isContinuing = true;
                    } else {
                        // Current talent reached 5, search for NEXT available talent < 5
                        const potentialTargets = [...newState.directors, ...newState.actors]
                            .filter(t => t.isDiscovered && t.bekanntheit < 5 && t.id !== talentId)
                            .sort((a,b) => a.id - b.id);

                        const nextTalent = potentialTargets.find(t => {
                            // Simplified busy check for internal automation
                            if (t.activeTraining) return false;
                            if (t.unavailableForProjectsUntil && newDate < new Date(t.unavailableForProjectsUntil)) return false;
                            // Check if talent is busy in OTHER casting missions
                            if (newState.activeCastings.some(c => c.talentId === t.id && c.casterId !== casterId)) return false;
                            return true;
                        });
                    ...
                    // Specific Casting: Continue with SAME talent until targetBekanntheit reached
                ...
                if (isGeneralCasting) {
                    if (isContinuing) {
                        // Distinguish between gaining a step and switching talent
                ...
                // Apply Continuation to list
                if (isContinuing) {
                    const caster = newState.employees.find(e => e.id === casterId);
                    const casterTalent = caster ? caster.talent : 50;
                    // Apply Effectiv Talent reduction (already in TalentManagementTab, but here for auto-continue)
                    // We assume basic speed calculation, but talent affects it.
                ...
                // Show notification for first/single completion only to avoid spam
                if (!castingUpdated) {
        ...
        if (castingUpdated) {
            ...
            newState.activeCasting = null; // Clear legacy
            // If any casting finished, pause
            systemPause();
        }

        // Casting Campaigns (Multiple)
        // Ensure array exists
        ...
        const remainingCampaigns: ActiveCastingCampaign[] = [];
        ...
        newState.activeCastingCampaigns.forEach(campaign => {
             if (dateReached(newDate, campaign.endDate)) {
                 ...
                 let minTalents = 2;
                let maxTalents = 3;
                if (scope === 'personal') { minTalents = 1; maxTalents = 2; }
                if (scope === 'medium') { minTalents = 3; maxTalents = 6; }
                if (scope === 'large') { minTalents = 8; maxTalents = 12; }
                
                const numTalents = Math.floor(Math.random() * (maxTalents - minTalents + 1)) + minTalents;
                const discovered: (Director | Actor)[] = [];
    
                // Calculate Effective Caster Quality
                const caster = newState.employees.find(e => e.id === casterId);
                let effectiveQuality = 10; // Fallback base
                if (caster) {
                    const satisfactionFactor = (caster.satisfaction || 100) / 100;
                    effectiveQuality = Math.round(caster.talent * satisfactionFactor);
                }
    
                // --- SKILL LEVEL LOGIC (New 5 Star Scale) ---
                let minSkill = 1;
                let maxSkill = 20;
                
                if (targetSkillLevel === 1) { // 0-1 Stars
                    minSkill = 1; maxSkill = 25; // Adjusted slightly for balance
                } else if (targetSkillLevel === 2) { // 2-3 Stars
                    minSkill = 20; maxSkill = 45;
                } else if (targetSkillLevel === 3) { // 4-5 Stars
                    minSkill = 40; maxSkill = 65;
                } else if (targetSkillLevel === 4) { // 6-7 Stars
                    minSkill = 60; maxSkill = 85;
                } else if (targetSkillLevel === 5) { // 8-10 Stars
                    minSkill = 80; maxSkill = 100;
                } else {
                ...
                // Track which IDs we found in this batch to avoid duplicates
                const foundTalentIdsInBatch = new Set<number>();
                ...
                for (let i = 0; i < numTalents; i++) {
                    ...
                    const candidates = pool.filter(t => {
                        ...
                        if (t.isDiscovered) return false;
                        // Must not be family or placeholder
                        if (t.id === -1 || (t as any).isFamily) return false;
                        // Must not have been picked in this batch already
                        if (foundTalentIdsInBatch.has(t.id)) return false;
                        // Skill Check
                        if (t.skill < minSkill || t.skill > maxSkill) return false;
                        
                        // Age Check (only for Actors)
                        if (!isDirector && targetAgeGroup) {
                            const age = getAge(t.birthDate, newState.gameDate);
                    ...
                    if (candidates.length > 0) {
                        ...
                        if (isDirector) {
                         ...
                         // Update local object for the list display
                        ...
                        // Note: We use a dummy scout talent here because we override skill manually below
                        const newTalent = generateNewTalent(newState.directors, newState.actors, undefined, undefined, targetRole, false, undefined, newDate);
                        
                        newTalent.isDiscovered = true;
                        newTalent.bekanntheit = 1; // Start with 1 Star Fame
                        
                        // Override Skill to match campaign promise
                        ...
                        // --- AGE GROUP LOGIC ---
                        let ageMin = 16, ageMax = 34; // Default Young
                        
                        if (isDirector) {
                            // Force MiddleAged or Old for Directors
                            const forceOld = Math.random() < 0.4; 
                            if (forceOld) { ageMin = 60; ageMax = 80; } 
                            else { ageMin = 35; ageMax = 59; }
                        } else {
                            // Standard Actor Logic based on user selection
                            if (targetAgeGroup === ActorAge.Child) { ageMin = 6; ageMax = 15; }
                            else if (targetAgeGroup === ActorAge.Young) { ageMin = 16; ageMax = 34; }
                            else if (targetAgeGroup === ActorAge.MiddleAged) { ageMin = 35; ageMax = 59; }
                            else if (targetAgeGroup === ActorAge.Old) { ageMin = 60; ageMax = 80; }
                        }
                        ...
                        const baseCost = isDirector 
                            ? 12000 + 8 * Math.pow(specificSkill, 3.1)
                            : 15000 + 10 * Math.pow(specificSkill, 3.1);
                        
                        newTalent.cost = Math.round(baseCost / 100) * 100;
        
                        if (isDirector) {
                            newState.directors = [...newState.directors, newTalent as Director];
                        } else {
                            newState.actors = [...newState.actors, newTalent as Actor];
                        }
        ...
        if (campaignUpdated) {
            ...
            newState.activeCastingCampaign = null; // Clear legacy
            systemPause();
        }

        // Talent Scouting (Single Scout) - Multiple
        ...
        const remainingScoutings: ActiveTalentScouting[] = [];
        ...
        newState.activeTalentScoutings.forEach(scouting => {
             if (dateReached(newDate, scouting.endDate)) {
                ...
                // IMPROVEMENT: Scout gains skill
                if (scout) {
                    newState.employees = newState.employees.map(e => 
                        e.id === scout.id ? { ...e, talent: Math.min(100, e.talent + 1) } : e
                    );
                }
        ...
        if (scoutingUpdated) {
            ...
            newState.activeTalentScouting = null; // Clear legacy
            systemPause();
        }

        // Talent/Employee Training
        const allPersonnel: (Director | Actor | Employee)[] = [...newState.directors, ...newState.actors, ...newState.employees];
        ...
        const updatedPersonnel = allPersonnel.map(person => {
            if (person.activeTraining && dateReached(newDate, person.activeTraining.endDate)) {
                let message = '';
                const updatedPerson = { ...person, activeTraining: undefined };
                const skillGain = 3 + Math.floor(Math.random() * 4);
                
                if ('skill' in updatedPerson) {
                    // DIRECTOR / ACTOR
                    const newSkill = Math.min(100, Math.min((updatedPerson as Director | Actor).potential, updatedPerson.skill + skillGain));
                    const actualGain = newSkill - updatedPerson.skill;
                    (updatedPerson as Director | Actor).skill = newSkill;
                     
                     // Message logic now handled via Message object, but for Talents we still trigger the info modal via setTrainingFinishedInfo (legacy/current UX for talent)
                    ...
                    (updatedPerson as Employee).talent = newTalent;
                    
                    // Update Cooldown Date
                    (updatedPerson as Employee).lastTrainingDate = new Date(newDate);

                    // Generate Email for Employee Training
                    // 1. Resolve Role Name
                    ...
                    const roleKey = typeMap[(updatedPerson as Employee).type];
                    // Fallback to type string if translation missing
                    const roleName = t.office.employees.employeeTypes[roleKey] || (updatedPerson as Employee).type;
                    ...
                    const newMessage: Message = {
                        ...
                        bodyTemplate: {
                            key: 'office.messages.trainingFinishedBodies', // Points to the array
                            variables: { 
                                name: person.name,
                                role: roleName,
        ...
        if (trainingFinished) {
            newState.directors = updatedPersonnel.filter(p => 'speedModifier' in p) as Director[];
            newState.actors = updatedPersonnel.filter(p => !('speedModifier' in p) && 'isDiscovered' in p) as Actor[];
            newState.employees = updatedPersonnel.filter(p => !('isDiscovered' in p)) as Employee[];
            dataChanged = true;
        }
        ...
        if (newState.activeWriting && dateReached(newDate, newState.activeWriting.endDate)) {
            ...
            // IMPROVEMENT: Writer gains skill
            if (writer) {
                newState.employees = newState.employees.map(e => 
                    e.id === writer.id ? { ...e, talent: Math.min(100, e.talent + 1) } : e
                );
            }

            // Calculate final quality based on writer talent
            const baseQuality = 30;
            const talentBonus = writer ? writer.talent * 0.5 : 0;
            const randomFactor = Math.floor(Math.random() * 21) - 10; // +/- 10
            ...
            // Role generation for self-written script
            const mainRoleGender: 'männlich' | 'weiblich' = Math.random() > 0.5 ? 'männlich' : 'weiblich';
            const supportingRoleGender: 'männlich' | 'weiblich' = Math.random() > 0.5 ? 'männlich' : 'weiblich';
            ...
            const newScript: Script = {
                ...
                price: Math.round((5000 + (finalQuality * finalQuality * 20)) / 100) * 100,
