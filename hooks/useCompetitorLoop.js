...
import { Genre, ProjectPhase } from '../types';
...
export const useCompetitorLoop = ({ playerData, setPlayerData }) => {
    ...
    useEffect(() => {
        setPlayerData(currentData => {
            ...
            let usedTitles = new Set(newState.competitors.flatMap(c => c.completedFilms.map(f => f.title)));
            // Helper to check if talent is busy in ANY active player project
            const isBusyInPlayerProject = (talentId) => {
