...
const useDateLoop = ({ setPlayerData, enabled = true }) => {
  ...
  const getUtcDayNumber = (date) => {
    return Math.floor(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate()) / (24 * 60 * 60 * 1e3));
  };
  const calculateNextRealtimeGameDate = (currentGameDate, elapsedRealMs) => {
