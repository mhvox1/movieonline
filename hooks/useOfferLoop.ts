...
import { PlayerData, ProjectData, Message, LifecycleOffer } from '../types';
...
const isSameDay = (d1: Date, d2: Date | undefined | string): boolean => {
    if (!d2) return false;
...
export const useOfferLoop = ({ playerData, setPlayerData, systemPause, pauseOnMessage }: any) => {
    ...
    useEffect(() => {
        setPlayerData((currentData: PlayerData) => {
                    ...
                    if (relevantMsg && relevantMsg.offerContext) {
                        ...
                        if (offer.status === 'active') {
                            ...
                            if (ctx.isNegotiationFailed) {
                                return { ...offer, status: 'failed' as const };
                            }
                            if (ctx.isWithdrawn) {
            ...
            const filmsToUpdate = newState.completedFilms.map(film => {
                ...
                if (film.activeDeal || !film.nextOfferDate) return film;
                ...
                if (newDate >= new Date(film.nextOfferDate)) {
                    ...
                    if (currentOffers.length === 0) {
                        ...
                        if (bestOffer) {
                            ...
                            if (strategyType === 'cinema_release') subjectKey = 'marketing.offerMessage.kinoSubject';
                            else if (strategyType === 'direct_to_video') subjectKey = 'marketing.offerMessage.homeSubject';
                            else if (strategyType === 'tv_premiere') subjectKey = 'marketing.offerMessage.payTvSubject';
                    ...
                    // Else: Add ONE new offer if available (Cycle 25-40 days)
                        ...
                        if (newOfferList.length > 0) {
                            ...
                            // Send "New Offer" Notification
                             ...
                             if (strategyType === 'direct_to_video') subjectKey = 'marketing.offerMessage.homeSubject';
                             else if (strategyType === 'tv_premiere') subjectKey = 'marketing.offerMessage.payTvSubject';
                             else if (strategyType === 'free_tv_dump') subjectKey = 'marketing.offerMessage.freeTvSubject';
                ...
                if (updatedFilm.offers && updatedFilm.offers.length > 0) {
                    ...
                    for (let i = activeOffers.length - 1; i >= 0; i--) {
                        ...
                        if (newDate >= new Date(offer.nextInteractionDate)) {
                            ...
                            let action: 'remind' | 'improve' | 'withdraw' | null = null;
                            ...
                            if (count === 0) {
                                ...
                                if (roll < 0.70) action = 'remind';      // 70%
                                else if (roll < 0.90) action = 'improve'; // 20%
                                else action = 'withdraw';                 // 10%
                            } 
                            // Now comes the 3rd Message (Count 1 -> 2)
                            else if (count === 1) {
                                // 3rd Message (approx 10-15 days later)
                                if (roll < 0.33) action = 'remind';
                                else if (roll < 0.66) action = 'improve';
