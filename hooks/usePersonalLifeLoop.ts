...
import { PlayerData, MaritalStatus, RandomEvent, WeddingDetails, EmployeeType, Message } from '../types';
import { isSameDay, dateReached } from './helpers';
import { WEDDING_DAY_EVENT, ALL_EVENTS, generateForcedBreakupEvent } from '../components/events';
...
interface UsePersonalLifeLoopProps {
    playerData: PlayerData;
    setPlayerData: React.Dispatch<React.SetStateAction<PlayerData | null>>;
    systemPause: () => void;
    setPregnancyNotification: React.Dispatch<React.SetStateAction<{ dueDate: Date; isAdoption: boolean } | null>>;
    setBirthModalData: React.Dispatch<React.SetStateAction<{ gender: 'Junge' | 'Mädchen'; isAdoption?: boolean } | null>>;
    setActiveEvent: React.Dispatch<React.SetStateAction<ActiveEventState | null>>;
    setGraduationModalData?: React.Dispatch<React.SetStateAction<{ childName: string; major: string; uniName: string; skillLevel: number } | null>>;
}
...
export const usePersonalLifeLoop = ({
    playerData,
    setPlayerData,
    systemPause,
    ...
    useEffect(() => {
        setPlayerData(currentData => {
            ...
            const isCommittedRelationship = 
                newState.maritalStatus === MaritalStatus.Dating || 
            ...
            if (newState.weddingDetails && dateReached(newDate, newState.weddingDetails.date)) {
                const details: WeddingDetails = newState.weddingDetails;
            ...
            if (newState.weddingDate && newState.maritalStatus === MaritalStatus.Married) {
                ...
                if (newDate.getDate() === weddingDate.getDate() && 
                    ...
                    const alreadyCelebrated = newState.eventLog.some(e => 
                        (e.title === eventTitle || e.title === "Wedding Anniversary") && 
