...
export const useProjectLoop = ({ playerData, setPlayerData, systemPause, }) => {
            ...
            if (newState.activePlanning) {
                if (newState.activePlanning.contract && newState.activePlanning.contractDeadline && newDate > new Date(newState.activePlanning.contractDeadline)) {
                    ...
                    if (newState.activePlanning.plannerId) {
                        newState.employees = newState.employees.map(e => e.id === newState.activePlanning.plannerId ? { ...e, talent: Math.min(100, e.talent + 1) } : e);
                    }
                ...
                for (let i = updatedProjects.length - 1; i >= 0; i--) {
                    ...
                    if (project.phase === ProjectPhase.Scriptwriting && newDate >= new Date(project.scriptEndDate)) {
                        ...
                        if (castingOption) {
                            ...
                            newState.actors = newState.actors.map(a => discoveredActorIds.has(a.id)
                                ? { ...a, isDiscovered: true, bekanntheit: Math.max(1, a.bekanntheit) }
                                : a);
                            newState.directors = newState.directors.map(d => discoveredDirectorIds.has(d.id)
                        ...
                        const weeklyCostsTransactions = newState.transactionLog.filter(t => t.category === 'Filmproduktion' &&
                        ...
                        const productionEventTransactions = newState.transactionLog.filter(t => project.productionStartDate &&
                        ...
                        const marketingCampaignTransactions = newState.transactionLog.filter(t => t.category === 'Marketing' &&
                            t.type === 'Ausgabe' &&
